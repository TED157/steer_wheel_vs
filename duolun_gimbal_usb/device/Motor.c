...
#define DaMiao_Motor_Enable_Message(message) \
    for (uint8_t i = 0; i < 7; i++)          \
        message[i] = 0xff;                   \
    message[7] = 0xfc;
#define DaMiao_Motor_Disable_Message(message) \
    for (uint8_t i = 0; i < 7; i++)           \
        message[i] = 0xff;                    \
    message[7] = 0xfd;
#define DaMiao_Motor_Error_Clear_Message(message) \
    for (uint8_t i = 0; i < 7; i++)               \
        message[i] = 0xff;                        \
    message[7] = 0xfb;
#define DaMiao_Motor_ZERO_SET_Message(message) \
    for (uint8_t i = 0; i < 7; i++)            \
        message[i] = 0xff;                     \
    message[7] = 0xfe;
...
void DaMiaoCanSend(float DaMiao)
{
    ...
    if (DamiaoPitchMotorMeasure.stall_flag == 1)
        DaMiao /= ((DaMiao > 0 ? DaMiao : -DaMiao) * 2);
...
void DaMiao_ctrl_Command(uint8_t *message, DaMiao_CMD_e CMD)
{
    ...
    {
    case DaMiao_DISABLE:
        DaMiao_Motor_Disable_Message(message) break;
    case DaMiao_ENABLE:
        DaMiao_Motor_Enable_Message(message) break;
    case DaMiao_CLear_ERROR:
        DaMiao_Motor_Error_Clear_Message(message) break;
    case DaMiao_Zero_SET:
        DaMiao_Motor_ZERO_SET_Message(message) break;
    default:
        break;
    }
}
...
void DaMiao_Motor_Stall_Monitor(DM_motor_t *DM_Motor)
{
    if ((DM_Motor->para.tor >= 1.0f || DM_Motor->para.tor <= -1.0f || DM_Motor->stall_flag == 1) && (DM_Motor->para.vel > 0 ? DM_Motor->para.vel : -DM_Motor->para.vel) < 0.2)
    {
        ...
        if (DM_Motor->stall_counter >= 65535)
            DM_Motor->stall_counter = 65535;
    }
